...
init_modules(const sim_params& s, const CodeParamsLDPC& c, modules& m)
{
    ...
    (*m.monitor)("check_errors").set_replicability(false);
...
init_utils(const sim_params& s, const modules& m, utils& u)
{
    u.pipeline.reset(
      spu::tools::Pipeline_builder()
        ...
        // ------------------------------------------------------------------------------------------------------------
        .add_stage(
          spu::tools::Pipeline_builder::Stage_builder() // ------------------------------------------- STAGE 0
            .set_first_tasks({ &(*m.QRNG)("generate"), &(*m.source)("generate") })

            .set_last_tasks({ &(*m.LLRs_computer)("compute_LLRs"), &(*m.syndrome_computer)("compute_syndrome") })
    ...
    std::ofstream f("test_pipeline.dot");
