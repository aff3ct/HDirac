...
struct modules
{
    ...
    std::unique_ptr<CVQKD::module::Octonion_operator<>> octonion_operator;
    // octonion multiplier and divider
    std::unique_ptr<CVQKD::module::LLRs_computer_low_dimensions<>>
      LLRs_computer_low_dimensions; // LLR computer for low reconciliation dimensions (d <= 8)
};
void
...
main(int argc, char** argv)
{
    ...
    std::cout << "#----------------------------------------------------------" << std::endl;
    std::cout << "#       ~~-- CV-QKD Virtual Channel [octonions] --~~       " << std::endl;
    std::cout << "#----------------------------------------------------------" << std::endl;
    std::cout << "#" << std::endl;
    ...
    (*m.octonion_operator)["divide::in_octonion_products"] = (*m.octonion_operator)["multiply::out_octonion_products"];
...
init_sim_params(sim_params& s)
{
    ...
    s._filename_ = "../data/h_matrices_bsparse/n204800r0p09765625_cil_RA_SPPCOM_2.bsparse";
    s.n_iter = 500;
    ...
    std::cout << "#    ** Reconcil. dim. = " << s.d << " [octonions]" << std::endl;
...
init_modules(const sim_params& s, const CodeParamsLDPC& c, modules& m)
{
    ...
    m.LLRs_computer_low_dimensions = std::unique_ptr<CVQKD::module::LLRs_computer_low_dimensions<>>(
      new CVQKD::module::LLRs_computer_low_dimensions<>(c.get_n(), s.d, s.sub_vec_length));
}
